...
```
cuid:azuredevops:{service}:{org}/{project}::{resource-type}/{resource-id}
```
...
→ cuid:azuredevops:projects:my-org::project/my-project
...
→ cuid:azuredevops:pipelines:my-org/my-project::pipeline/42

dev.azure.com/my-org/my-project/_workitems/edit/1234 (work item)
→ cuid:azuredevops:boards:my-org/my-project::work-item/1234

dev.azure.com/my-org/my-project/_environments/5 (environment)
→ cuid:azuredevops:environments:my-org/my-project::environment/5

dev.azure.com/my-org/my-project/_artifacts/feed/my-feed (artifact feed)
→ cuid:azuredevops:artifacts:my-org/my-project::feed/my-feed

my-org (organization)
→ cuid:azuredevops:orgs:my-org::org/my-org
```
...
→ cuid:azuredevops:repos:my-collection/my-project:tfs.company.com:repository/my-repo
```
...
|---------------------|-------------------------------------------------------------------------|
| Repository          | `cuid:azuredevops:repos:{org}/{proj}::repository/{name}`               |
| Branch              | `cuid:azuredevops:repos:{org}/{proj}::branch/{repo}/{branch}`          |
| Tag                 | `cuid:azuredevops:repos:{org}/{proj}::tag/{repo}/{tag}`                |
| Commit              | `cuid:azuredevops:repos:{org}/{proj}::commit/{repo}/{sha}`             |
| Pull Request        | `cuid:azuredevops:repos:{org}/{proj}::pull-request/{repo}/{id}`        |
| PR Thread           | `cuid:azuredevops:repos:{org}/{proj}::thread/{repo}/{pr-id}/{id}`      |

### Pipelines & CI/CD

| Resource Type       | CUID Example                                                            |
|---------------------|-------------------------------------------------------------------------|
| Pipeline (def)      | `cuid:azuredevops:pipelines:{org}/{proj}::pipeline/{id}`               |
| Pipeline Run        | `cuid:azuredevops:pipelines:{org}/{proj}::run/{pipeline-id}/{run-id}`  |
| Release Definition  | `cuid:azuredevops:releases:{org}/{proj}::release-def/{id}`             |
| Release             | `cuid:azuredevops:releases:{org}/{proj}::release/{def-id}/{id}`        |
| Environment         | `cuid:azuredevops:environments:{org}/{proj}::environment/{id}`         |
| Variable Group      | `cuid:azuredevops:pipelines:{org}/{proj}::variable-group/{id}`         |
| Service Connection  | `cuid:azuredevops:connections:{org}/{proj}::service-connection/{id}`   |

### Work Tracking

| Resource Type       | CUID Example                                                            |
|---------------------|-------------------------------------------------------------------------|
| Work Item           | `cuid:azuredevops:boards:{org}/{proj}::work-item/{id}`                 |
| Board               | `cuid:azuredevops:boards:{org}/{proj}::board/{name}`                   |
| Sprint / Iteration  | `cuid:azuredevops:boards:{org}/{proj}::iteration/{path}`               |
| Area                | `cuid:azuredevops:boards:{org}/{proj}::area/{path}`                    |
| Query               | `cuid:azuredevops:boards:{org}/{proj}::query/{id}`                     |

### Artifacts

| Resource Type       | CUID Example                                                            |
|---------------------|-------------------------------------------------------------------------|
| Feed                | `cuid:azuredevops:artifacts:{org}/{proj}::feed/{name}`                 |
| Package             | `cuid:azuredevops:artifacts:{org}/{proj}::package/{feed}/{name}`       |
| Package Version     | `cuid:azuredevops:artifacts:{org}/{proj}::version/{feed}/{pkg}/{ver}`  |

### Organization

| Resource Type       | CUID Example                                                            |
|---------------------|-------------------------------------------------------------------------|
| Organization        | `cuid:azuredevops:orgs:{org}::org/{org}`                               |
| Project             | `cuid:azuredevops:projects:{org}::project/{name}`                      |
| Team                | `cuid:azuredevops:projects:{org}/{proj}::team/{name}`                  |
| Wiki                | `cuid:azuredevops:wiki:{org}/{proj}::wiki/{name}`                      |

### Test

| Resource Type       | CUID Example                                                            |
|---------------------|-------------------------------------------------------------------------|
| Test Plan           | `cuid:azuredevops:test:{org}/{proj}::test-plan/{id}`                   |
| Test Suite          | `cuid:azuredevops:test:{org}/{proj}::test-suite/{plan-id}/{id}`        |
| Test Case           | `cuid:azuredevops:test:{org}/{proj}::test-case/{id}`                   |
...
4. **TFVC repositories** — Azure DevOps supports TFVC (centralized VCS) alongside Git. TFVC repos use `$/project` paths. CUID: `cuid:azuredevops:repos:{org}/{proj}::tfvc-path/{path}`.
