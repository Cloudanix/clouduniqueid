...
def get_role_id(roleName: str, projectId: str) -> str:
    if roleName and projectId:
        ...
        raise ValueError(f"Invalid role name provided, GCP role name and projectId required")

    else:
        logger.error("GCP roleName and projectId required")
        raise ValueError(f"Invalid parameters provided, GCP role name and projectId required")


class GCPUniqueId:
    def get_unique_id(
        self, service: str, resourceType: str, data: Dict, projectId: str = None,
        location: str = None,
    ) -> str:
        ...
        if not uniqueIds.get(service, None):
            ...
            logger.error(f"GCP service {service} resource type {resourceType} not supported")
            raise ValueError(f"GCP service {service} resource type {resourceType} not supported")
